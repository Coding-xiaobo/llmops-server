...
class ConversationSummaryBufferMemory:
    """摘要缓冲混合记忆类
    核心思想：
    判断chat_history（summary+多轮对话记录）的长度是否大于token限制
    如果大于了，使用多轮对话的第一个，去更新摘要的内容，重新组成chat_history
    """

    # 1.max_tokens用于判断是否需要生成新的摘要
    # 2.summary用于存储摘要的信息
    # 3.chat_histories用于存储历史对话（summary+多轮对话记录）
    ...
    def __init__(self, summary: str = '', chat_histories: list = None, max_tokens: int = 300):
        ...
        self._client = OpenAI(base_url='https://api.xty.app/v1')  # 使用大模型去处理摘要
    ...
    def save_context(self, human_query: str, ai_content: str) -> None:
        ...
        # 对话历史已超最大token 使用最远的一次对话更新摘要，然后更新历史对话
